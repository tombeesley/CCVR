...
outlier_perc_removed <- 
  perc_removed %>% 
  filter(!between(z_perc, -2.5, 2.5)) %>%  # one P detected
  pull(subj)

# remove the outlier participant from the sample
data <- filter(data,!subj %in% outlier_perc_removed)
...
data_RT_ANOVA_sub_exp <- 
  data_RT_ANOVA_sub_exp %>% 
  as_tibble() %>% 
  mutate(across(c(sub_exp,TT,targ_depth,epoch), as.factor))

anovaBF(formula = meanRT ~ TT*targ_depth*epoch*sub_exp + subj,
        whichRandom = "subj",
        data = data_RT_ANOVA_sub_exp)
...
# output to check in JASP
data_RT_ANOVA %>% 
  pivot_wider(names_from = c("epoch", "TT", "targ_depth"), values_from = meanRT) %>% 
  write_csv("jasp.csv")
