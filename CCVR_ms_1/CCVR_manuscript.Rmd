...
The above analysis suggests that the target-distractor contingency factor did not play a significant role in determining reaction times. To confirm this, a Bayesian ANOVA was conducted with factors of *configuration type*, *target-depth*, and *target-distractor contingency*. This revealed considerable support for a main effect of *configuration type*, BF~10~ = `r spec_dec(extractBF(exp_2_anova_BF_a[2])$bf,1)`, and *target-depth*, BF~10~ = `r spec_dec(extractBF(exp_2_anova_BF_a[5])$bf,1)`. In contrast, there was moderate support for the absence of an effect of *target-distractor contingency*, BF~10 = `r spec_dec(extractBF(exp_2_anova_BF_a[1])$bf,1)`. The model which produced the largest Bayes Factor was that containing only factors of *configuration type* and *target-depth* with no interaction effects, BF~10 = `r spec_dec(extractBF(exp_2_anova_BF_a[7])$bf,1)`, and there was support for the absence of an interaction between these two factors, BF~10 = `r spec_dec(extractBF(exp_2_anova_BF_a[13])$bf/extractBF(exp_2_anova_BF_a[7])$bf,1)`. We next explored the possible interaction effects involving *target-distractor contingency*. There was support for the absence of an interaction effect between *target-distractor contingency* and *configuration type*, BF~10 = `r spec_dec(extractBF(exp_2_anova_BF_a[4])$bf/extractBF(exp_2_anova_BF_a[3])$bf,1)`, and also between *target-distractor contingency* and *target-depth*, BF~10 = `r spec_dec(extractBF(exp_2_anova_BF_a[10])$bf/extractBF(exp_2_anova_BF_a[6])$bf,1)`. There was also support for the absence of a three-way interaction effect, BF~10 = `r spec_dec(extractBF(exp_2_anova_BF_a[18])$bf/extractBF(exp_2_anova_BF_a[17])$bf,1)`. 

An additional Bayesian ANOVA was run with factors of *configuration type*, *target-depth*, and *test* to explore the three-way interaction depicted in Figure \@ref(fig:Exp2-CC-figure), which was significant in the ANOVA reported earlier. This revealed a Bayes Factor for the interaction that did not amount to supporting evidence, BF~10 = `r  spec_dec(extractBF(exp_2_anova_BF_b[18])$bf/extractBF(exp_2_anova_BF_b[17])$bf,1)`. 
